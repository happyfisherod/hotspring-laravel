...
@section("content")
 ...
 <div class="sub-banner" style="background:url({{ URL::asset('upload/'.getcong('page_bg_image')) }}) no-repeat center top;">
    <div class="overlay">
      <div class="container">
        <h1>{{ trans('messages.Order Confirmed') }}</h1>
      </div>
    </div>
  </div>   
      ...
      <div class="row">
        ...
        <div class="col-md-6">
          <div class="box_style_2">
            <h2 class="inner">{{ trans('messages.Order Confirmed') }}</h2>
            <div id="confirm"> <i class="fa fa-check-square-o"></i>
              <h3>{{ trans('messages.Thank you!') }}</h3>
            </div>
            <h4>{{ trans('messages.Summary') }}</h4>
            <table class="table table-striped nomargin">
              <tbody>
                <div style="display:none;">{{ $sum_fee = 0 }}</div>
                @foreach(\App\Cart::where('user_id',Auth::id())->orderBy('id')->get() as $n=>$order_item)
                <div style="display:none;">{{ $item_fee = $order_item->kind==2 ? 0 : $order_item->quantity * getcong('delifee') }}</div>
                <div style="display:none;">{{ $sum_fee += $item_fee }}</div>
                <tr>
                    <td><strong>{{$order_item->quantity}}x</strong> {{$order_item->item_name}} </td>
                    <td><strong class="pull-right">{{getcong('currency_symbol')}}{{$order_item->item_price + $item_fee}}</strong></td>
                </tr>
                     
                @endforeach

                <tr>
                  <td class="total_confirm"> {{ trans('messages.TOTAL') }} </td>
                  <td class="total_confirm"><span class="pull-right">{{getcong('currency_symbol')}}{{$price = DB::table('cart')
                ->where('user_id', Auth::id())
                ->sum('item_price') + $sum_fee}}</span></td>
                </tr>

              </tbody>
            </table>
            <a href="{{URL::to('myorder')}}" class="btn btn-submit">{{ trans('messages.My Order') }}</a> 
