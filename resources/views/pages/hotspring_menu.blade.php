...
@section('head_title', $hotspring->hotspring_name.' Menu' .' | '.getcong('site_name') )
...
@section("content")
  ...
  <div class="sub-banner" style="background:url({{ URL::asset('upload/'.getcong('page_bg_image')) }}) no-repeat center top;">
    ...
    <div class="col-md-2 col-sm-3">
      <div id="thumb"><img src="{{ URL::asset('upload/hotspring/'.$hotspring->hotspring_logo.'-b.jpg') }}" alt="{{ $hotspring->hotspring_name }}"></div>
    </div>  
    <div class="col-md-10 col-sm-9">  
      <h1>{{ $hotspring->hotspring_name }}</h1>
      <div class="sub_cont_rt">{{ $hotspring->type }}</div>
      ...
      <div class="rstl_list_btn"><a href="{{URL::to('hotspring/'.$hotspring->hotspring_slug)}}">{{ trans('messages.View HotSpring')}}</a></div>
      </div>
    </div>
      </div>
    </div>
  ...
  <div class="hotspring_list_detail">
    <div class="container">
      ...
      <div id="main_menu" class="box_style_2">
        <h2 class="inner">{{trans('messages.HotSpring List')}}</h2>
        @foreach($categories as $n=>$cat)
        <h3 id="{{$cat->category_name}}" class="nomargin_top">{{$cat->category_name}}</h3>
        <table class="table table-striped cart-list">
          <thead>
          <tr>
            <th width="70%">{{ trans('messages.Item')}}</th>
            <th width="15%">{{ trans('messages.Price')}}</th>
            <th width="15%">{{ trans('messages.Order')}}</th>
          </tr>
          </thead>
          <tbody>
